...
apps/admin/
  ...
  src/
    utils/frame/          渲染引擎(本 app 的核心资产,禁止 import React/arco/store)
      ...
      export-pipeline.ts  批量渲染 + zip 累积 + 进度回报
    ...
    routes/               约定式路由:layout.tsx(壳)、frames/page.tsx、frames/$styleId/export/page.tsx
    types.ts              清单数据的运行时形状
...
- **React 19 前置**:`src/routes/layout.tsx` 第一行必须 `import "@arco-design/web-react/es/_util/react-19-adapter"`,否则 `Message`/`Notification` 直接崩;
...
- **移动端是一等公民**(< 768px):
  ...
  - 移动端批量 >20 张时进度弹框给一条内存/耗时软提示,不阻断。
...
- 两者覆盖不了才自写;
- **Blob URL / fetch / Worker 三件事不允许在业务代码里裸调**,必须经 `src/utils/`(分别是 `download.ts`、`catalog.ts`、`utils/frame/worker-pool.ts`)。
...
```bash
pnpm --filter @monorepo-template/admin dev        # 本地开发
pnpm --filter @monorepo-template/admin build      # 产物在 apps/admin/dist
pnpm build:pages                                  # 按 Pages 子路径构建 + 产出 404.html
```
...
| 关键流程 | `tests/playwright/watermark-frame.spec.ts`:选图 → 导出 → 拿到 zip 一条黄金路径                         |
